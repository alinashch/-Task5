...
public class SimpleBinaryTree<T> implements BinaryTree<T> {
    ...
    public SimpleTreeNode Solution(int[] arr, int start, int end) {
        if (start > end) {
            return null;
        }
        SimpleBinaryTree<Integer> Tree = new SimpleBinaryTree<>(Integer::parseInt);
        int mid = (start + end) / 2;
        SimpleBinaryTree.SimpleTreeNode Node = new SimpleBinaryTree.SimpleTreeNode(arr[mid]);
        Node.left=Solution(arr,start,mid-1);
        Node.right=Solution(arr,mid+1, end);
        this.root=Node;
        return Node;
    }
    public void er(int[] arr) throws Exception {
        SimpleTreeNode root = Solution(arr,0, arr.length-1);
        this.root = root;
    }
